...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Owsiak, Erik");
MODULE_DESCRIPTION("SoftUART for RPi");
MODULE_VERSION("0.1");
...
static int gpio_tx = 8;
module_param(gpio_tx, int, 0);

static int gpio_rx = 25;
...
static int __init soft_uart_init(void)
{
  ...
  soft_uart_driver->driver_name           = "uart_gsm";
  soft_uart_driver->name                  = "ttyGSM";
  ...
  /* speed */
  soft_uart_driver->init_termios.c_ispeed = baud;
  soft_uart_driver->init_termios.c_ospeed = baud;
 ...
 * @param clear
 */
static int soft_uart_tiocmset(struct tty_struct* tty, 
 ...
 * @param parameter
 */
static int soft_uart_ioctl(struct tty_struct* tty, 
	unsigned int command, unsigned int long parameter)
{
  int error = NONE;
